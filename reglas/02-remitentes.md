...
| `comfy-cloud@news.comfy.org` | Newsletter. Visto el 28/08/2026 con el anuncio de Gemini Omni 1.1 Flash. **Tercera combinacion de Comfy, y la que prueba que lo que va antes de la arroba no predice nada:** decide el subdominio. `news.comfy.org` es newsletter y se archiva, `updates.comfy.org` es cuenta y se archiva informativo, y una direccion `@comfy.org` sin subdominio es una persona y es partner |
| `timothy@elevenlabs.io` | Marketing de afiliados de ElevenLabs, enviado por PartnerStack en nombre de Eleven Labs Inc. Empuja a publicar contenido dentro de una ventana de 24 a 48 horas usando el link de referido del estudio. Visto el 28/08/2026, duplicado en el mismo hilo. **No confundir con `no-reply@elevenlabs.io`**, que son alertas de la plataforma. Se archiva como promo, pero ver la duda del registro del 28/08: ese link de referido implica que hay una cuenta de afiliado a nombre del estudio que este repositorio no tenia anotada |
...
| `businessprofile-noreply@google.com` | Informes de rendimiento del perfil del estudio. **Excepción:** si es una invitación a administrar el perfil de un cliente, o una opinión sobre un perfil de cliente, no es ruido, va a `ACCION REQUERIDA`. La excepción se disparo por primera vez el 24/08/2026, con una opinion de 1 estrella en el perfil de Otro Mundo, y funciono. El aviso de Google **trae el texto de la opinion cortado**, asi que en el resumen se cita solo lo que se ve y se aclara que esta cortado, nunca se completa la queja por inferencia. **Precision del 28/08/2026, dos cosas que la ficha no cubria.** Primero, el caso base dice "perfil del estudio", asi que un aviso informativo sobre el perfil de un **cliente** no estaba en ninguna categoria: una foto nueva subida por un visitante, o un informe de rendimiento, es ruido y se archiva; una opinion o un cambio de permisos no. Segundo, la excepcion no distingue opiniones buenas de malas, y el 28/08 puso `ACCION REQUERIDA` a una calificacion de 5 estrellas sin texto, que no necesita ninguna accion. Mientras no se cambie la regla se sigue etiquetando toda opinion, pero **en el resumen se aclara la cantidad de estrellas**, porque no es lo mismo un reclamo que un elogio. **Excepción a la excepción, instrucción explícita de Nicolás el 14/09/2026: para el perfil de Otro Mundo puntualmente, ninguna opinión ni comentario se etiqueta `ACCION REQUERIDA` ni se deja para revisión.** No le interesa verlas ni responderlas, así que se archivan directo, sin etiqueta y sin entrar al resumen individualmente, sea cual sea la cantidad de estrellas o el contenido. Esto pisa el criterio general de arriba solo para Otro Mundo; para cualquier otro cliente sigue rigiendo la excepción original (`ACCION REQUERIDA` con la cantidad de estrellas aclarada) |
...
| `gemini-notes@google.com` | Notas de reunion generadas por Gemini. **No requiere accion de la corrida:** hay un filtro nativo de Gmail viejo que ya las manda a la etiqueta `Gemini Notes` fuera del inbox, con 258 mensajes acumulados al 28/08/2026. Se anota para que ninguna corrida futura las levante como sin clasificar si el filtro alguna vez se cae | Ninguna |
